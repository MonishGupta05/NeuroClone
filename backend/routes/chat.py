...
from services.memory_service import (
    get_memory_context, store_memory,
    store_correction, auto_tag_memory
)
...
def chat(request: ChatRequest, db: Session = Depends(get_db)):
    ...
    mem_type, importance = auto_tag_memory(request.message)
    if mem_type != "conversation" or len(request.message) > 30:
        store_memory(
            db,
    ...
    return {"response": response, "memory_tagged": mem_type}
...
3. Any important facts about him

Conversation:
{request.conversation}

Reply with just the summary, no labels, no bullet points."""
